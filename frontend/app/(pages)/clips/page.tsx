...
export default function ClipsPage() {
  ...
  // Improved audio playback functions
  const playAudio = async (reelId: string) => {
    ...
    if (playPromisesRef.current[reelId]) {
      try {
        await playPromisesRef.current[reelId];
      } catch (e) {
        // Ignore - we just want to wait for it to complete
      }
  ...
  const pauseAudio = (reelId: string) => {
    const audio = audioRefs.current[reelId];
    if (!audio) return;
  ...
  const handleLike = async (reelId: string) => {
    // Prevent multiple simultaneous like/unlike requests for the same clip
    if (likingInProgress[reelId]) {
      return;
    }
    ...
    // Store original state for proper reversion
    const originalLikeCount = currentReel.likes;
    const originalLikedState = wasLiked;

    // Mark as in progress
    setLikingInProgress(prev => ({ ...prev, [reelId]: true }));
    ...
    try {
      ...
      if (!result.success) {
        ...
        setLikedReels(prev => ({ ...prev, [reelId]: originalLikedState }));
        ...
        // Ensure liked state matches what we expect
        setLikedReels(prev => ({ ...prev, [reelId]: !wasLiked }));
      }
    } catch (error) {
      ...
      setLikingInProgress(prev => ({ ...prev, [reelId]: false }));
    }
  };
  ...
  "
          ...
          {/* Mobile Comments Popup - continues with same pattern... */}
          {/* I'll skip the mobile comments section for brevity, but it follows the same pattern */}

...
export default function AchievementsPopup({
  ...
  const [organization, setOrganization] = useState(existingData?.organization || '');
  const [dateAwarded, setDateAwarded] = useState(existingData?.dateAwarded || '');
  const [sport, setSport] = useState(existingData?.sport || '');
  const [positionEvent, setPositionEvent] = useState(existingData?.positionEvent || '');
  const [achievementType, setAchievementType] = useState(existingData?.achievementType || '');
  ...
  const [mediaPdfName, setMediaPdfName] = useState(existingData?.mediaPdf ? (typeof existingData.mediaPdf === 'string' ? existingData.mediaPdf : existingData.mediaPdf.name) : '');

  const [showSportDropdown, setShowSportDropdown] = useState(false);
  const [showAchievementTypeDropdown, setShowAchievementTypeDropdown] = useState(false);
  ...
  useEffect(() => {
    if (open && existingData) {
      ...
      setMediaPdfName(existingData.mediaPdf ? (typeof existingData.mediaPdf === 'string' ? existingData.mediaPdf : existingData.mediaPdf.name) : '');
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (achievementTypeRef.current && !achievementTypeRef.current.contains(event.target as Node)) {
  ...
  const handleSave = () => {
    ...
    if (!title.trim() || !organization.trim() || !dateAwarded || !sport || 
        !positionEvent.trim() || !achievementType || !level || !location.trim() || 
        !description.trim()) {
  ...
  const isFormValid = title.trim() && organization.trim() && dateAwarded && sport && 
                      positionEvent.trim() && achievementType && level && location.trim() && 
                      description.trim();
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative z-10 w-full max-w-2xl bg-white rounded-xl shadow-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="sticky top-0 z-10 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
          <h2 className="text-2xl font-bold text-gray-900">{existingData ? 'Edit Achievement' : 'Add Achievements'}</h2>
          <button
            onClick={onClose}
        ...
        <div className="px-6 py-6 space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Position / Event</label>
            <input
              type="text"
              value={positionEvent}
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Media Upload (PDF only)</label>
            <button

...
export function useMention({ currentUserId, value, onChange, onSelectionChange }: UseMentionOptions) {
  ...
  const handleInputChange = useCallback((
    ...
    const pos = cursorPos !== undefined ? cursorPos : e.target.selectionStart || 0;
  ...
  const insertMention = useCallback((follower: Follower) => {
    ...
    if (mentionState.showSuggestions && mentionState.suggestions.length > 0) {
      if (e.key === 'ArrowDown') {
        ...
        setMentionState(prev => ({
          ...prev,
          selectedIndex: prev.selectedIndex < prev.suggestions.length - 1 ? prev.selectedIndex + 1 : prev.selectedIndex,
        }));

    ...
    const fetchUserData = async () => {
      try {
        ...
        if (username) {
          ...
          response = await fetch(
            `${process.env.NEXT_PUBLIC_API_URL || 'https://athlinked-api.randomw.dev/api'}/signup/user/${encodeURIComponent(email)}`
          );
        }
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || 'https://athlinked-api.randomw.dev/api'}/signup/parent-complete`,
        {
          method: 'POST',
          headers: {
